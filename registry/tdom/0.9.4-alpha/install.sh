...
# tdom
if true; then
  curl -L -o tdom-trunk-src.tar.gz --output-dir $BUILD_DIR http://tdom.org/index.html/tarball/trunk/tdom-trunk.tar.gz
  tar -xzf $BUILD_DIR/tdom-trunk-src.tar.gz -C $BUILD_DIR
  cd $BUILD_DIR/tdom-trunk/unix
